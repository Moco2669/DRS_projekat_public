...
app = Flask(__name__)
CORS(app, origins="*", methods=["POST", "GET", "PUT"])
...
def get_user_info():
    ...
    if user.exists:
        user = user.to_dict()
        bill = bill.to_dict()
        card = db.collection("CreditCards").document(user["cardNum"]).get()
        card = card.to_dict()
        ...
        if user["cardNum"] != "":
            is_card_added = True
            admin_approve = card["admin_approve"]
        return jsonify({"bill": bill, "name": f"{user[name]} {user[lastName]}", "is_admin": is_admin,
                        "is_verified": admin_approve, "is_card_added": is_card_added}), 200
    return jsonify({"message": "User not found"}), 404
...
def order_processing():
    try:
        while True:
            print("OVAJ PROCES JE PUSTEN I ULAZI UNUTAR PETLJE")
            ...
            for order in incomplete_orders:
                ...
                product_ref = firestore.client().collection("Products").document(order_data.get("productId"))
                product_data = product_ref.get().to_dict()
                ...
                send_simple_message("lukadjelic529@gmail.com", "Order(s) update", message)
